...
调用 `ZMessageBox.alert` 方法以打开 alert 框。
...
调用 `ZMessageBox.confirm` 方法以打开 confirm 框。
...
调用 `ZMessageBox.prompt` 方法以打开 prompt 框。
...
可以通过配置 `ZMessageBox` 的参数来实现一些个性化需求（options 参数见下文）。
...
`ZMessageBox` 提供了全局方法 `ZMessageBox.alert`、`ZMessageBox.confirm`、`ZMessageBox.prompt`，用于在需要时弹出消息弹框。
...
```typescript
import { ZMessageBox } from "zm-element";
```
