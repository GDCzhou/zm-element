...
describe("createMessage", () => {
  test("调用方法应该创建对应的 Message 组件", async () => {
    ...
    await rAF();
    expect(document.querySelector(".z-message")).toBeTruthy();
    handler.close();
    await rAF();
    expect(document.querySelector(".z-message")).toBeFalsy();
  });
  ...
  test("多次调用应该创建多个实例", async () => {
    ...
    closeAll();
    await rAF();
    expect(document.querySelectorAll(".z-message").length).toBe(0);
  });
  ...
  test("创建多个实例应该设置正确的 offset", async () => {
    ...
    await rAF();
    const elements = document.querySelectorAll(".z-message");
