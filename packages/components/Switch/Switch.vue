...
defineOptions({ name: "ZSwitch", inheritAttrs: false });
...
watch(checked, (val) => {
  ...
  formItem?.validate("change").catch((err) => debugWarning(err));
});
watch(
  () => props.modelValue,
...
<template>
  ...
  >
    <input
      ...
      @blur="formItem?.validate('blur').catch((err) => debugWarning(err))"
    />
    <div class="z-switch__core">
      <div class="z-switch__core-inner">
        <span
          v-if="activeText || inactiveText"
          class="z-switch__core-inner-text"
        >
